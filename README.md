...
本番環境のデータベースに変更操作を加える場合、バグや操作ミスで本番のデータが失われるのを避けるために、バックアップを取ってから作業を行う事が望ましい。デプロイ作業を行う前にもバックアップを取ると安心である。

以下のスクリプトを実行すると、バックアップが取れる。対象のテーブルは、customersとproductsとsalesとjoined_salesであり、それぞれの名前に "_bak" がついたテーブルが作成される。

```bash
./scripts/backup_tables.sh --save bigquery-study-458607
```

その上で、任意の変更作業やデプロイ作業を行う。もし何らかの不具合が発生した場合には、以下のコマンドでバックアップデータを元のテーブルにコピーする。

```bash
./scripts/backup_tables.sh --restore bigquery-study-458607
```

全ての操作が成功していることが確認できたら、バックアップデータを消す。
...
ただし、タイムトラベルではPARTITION/CLUSTER/OPTIONSなどのメタデータは復元されず、また復元の時刻を把握するのが煩雑になる。よって、実行コストが許すのであれば明示的にバックアップを取った方が楽である。
